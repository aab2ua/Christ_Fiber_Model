...
if __name__ == "__main__":
    ...
    try:
        og_path = "/Users/aartheebaskaran/PycharmProjects/Fiber_Quantification_Christ"
        os.chdir(og_path)
        os.chdir(args.files[0])
        file_list = os.listdir()
        print(file_list)
        for i in range(6, len(file_list)):
            new_folder = file_list[i] + " analysis"  # make sure folder isn't already existing in directory - WILL CAUSE ERROR
            if file_list[i] != ".DS_Store" and os.path.isdir(new_folder) == False:
                print(file_list[i])
                print('Reading input file...')
                mask_name = file_list[i]
                ...
                print('Fiber Count')
                filt_mask = Image.fromarray(filtered_mask)
                filt_mask.save("filtered_mask.jpeg")
                print('a')
        ...
        print("Folder already exists - making directory issue")
